...
export const ProductDetail = ({
  ...
  React.useEffect(() => {
    ...
    setGroups(
      orderedGroups.filter((group) =>
        product.complementsGroupsIds.includes(group.id)
      )
    );
  ...
  React.useEffect(() => {
    ...
    getDownloadURLByPath(
      `businesses/${businessId}/products/${productId}_1008x720.jpg`,
      setImageUrl
    );
      ...
      <Box>
        <Box position="relative" w="100%" mb={{ lg: "2" }}>
          ...
          <Flex
            ...
            alignItems="center"
          >
            <Text ml={{ lg: "120px" }} fontSize="md" lineHeight="1.5rem">
              {businessName}
            </Text>
          </Flex>
        </Box>
  ...
  return (
    <Box>
      <Box position="relative" w="100%" mb={{ lg: "2" }}>
        <Flex position="absolute" top="0" left="0" alignItems="center" h="48px">
          <ArrowBackIcon
            display={{ lg: "none" }}
            w="20px"
            h="20px"
      ...
      >
        ...
        >
          <Box maxW={{ base: "228px", lg: "400px" }}>
            ...
            <Text mt="1" color="#697667" fontSize="md" lineHeight="1.25rem">
              {product?.description ??
                "Não foi possível carregar a descrição do produto"}
            </Text>
            <Text mt="1" fontSize="md">
              {groups?.length > 0 && "A partir de "}
      ...
      {groups &&
        groups.map((group) => (
          <ComplementsGroupItem key={group.id} group={group} />
        ))}
    </Box>
  );
};
