...
export const getBusinessesSlugs = async () => {
  ...
  return await getDocs(q).then((res) => {
    if (!res.empty) {
      const slugs = getBusinessesObject(res.docs).map(
...
export const getDownloadURLByPath = (
  path: string,
  resultHandler: (url: string | null) => void
) => {
...
// formatters
export const getBusinessObject = (
  docs: QueryDocumentSnapshot<DocumentData>[]
): DomainBusiness => {
  const businesses = docs.map((doc) => {
    const docData = doc.data() as Business;
    const phone =
      docData.phones.find((phone) => phone.type === "desk")?.number ??
      docData.phones.find((phone) => phone.type === "manager")?.number ??
      docData.phones.find((phone) => phone.type === "owner")?.number ??
      "N/E";
    return {
      id: doc.id,
      cnpj: docData.cnpj,
...
export const getBusinessesObject = (
  docs: QueryDocumentSnapshot<DocumentData>[]
): DomainBusiness[] => {
  const businesses = docs.map((doc) => {
    const docData = doc.data() as Business;
...
export const getCategoriesObjects = (
  docs: FirebaseDocument[]
): WithId<Category>[] => {
...
export const getProductsObjects = (
  docs: FirebaseDocument[]
): WithId<Product>[] => {
  return docs.map((item) => {
    const {
...
export const getComplementsObjects = (
  docs: FirebaseDocument[]
): WithId<Complement>[] => {
  return docs.map((item) => {
